...
| `bundleURL`        | `URL?`            | `nil`           | Custom bundle URL for direct file system access.          |
| `onBundleReload`   | `(() -> Void)?`   | `nil`           | Callback invoked when React Native reload is triggered.   |
...
### UIKit AppDelegate Approach (Traditional)
